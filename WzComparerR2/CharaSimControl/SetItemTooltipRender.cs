...
namespace WzComparerR2.CharaSimControl
{
    public class SetItemTooltipRender : TooltipRender
    {
        ...
        private Bitmap RenderSetItem(bool specialPetSetEffectName, out int picHeight)
        {
            ...
            picHeight = 10;
            TextRenderer.DrawText(g, this.SetItem.SetItemName, Translator.IsKoreanStringPresent(this.SetItem.SetItemName) ? GearGraphics.KMSItemDetailFont : GearGraphics.ItemDetailFont2, new Point(261, 10), ((SolidBrush)GearGraphics.GreenBrush2).Color, TextFormatFlags.HorizontalCenter);
            picHeight += 25;
            ...
            if (this.SetItem.SetItemID > 0)
            {
                ...
                foreach (var setItemPart in this.SetItem.ItemIDs.Parts)
                {
                    ...
                    if (!partNames.Contains(itemName + typeName))
                    {
                        ...
                        if (!cash)
                        {
                            ...
                            TextRenderer.DrawText(g, Compact(g, itemName, 261 - 12 - typeWidth - 10), Translator.IsKoreanStringPresent(itemName) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipDetailFont2, new Point(10, picHeight), ((SolidBrush)brush).Color, TextFormatFlags.NoPadding);
                            picHeight += 18;
                        }
                        else
                        {
                            ...
                            int typeWidth = TextRenderer.MeasureText(g, typeName, GearGraphics.EquipDetailFont2, new Size(int.MaxValue, int.MaxValue), TextFormatFlags.NoPadding).Width;
                            TextRenderer.DrawText(g, typeName, GearGraphics.EquipDetailFont2, new Point(261 - 10 - typeWidth, picHeight), ((SolidBrush)brush).Color, TextFormatFlags.NoPadding);
                            TextRenderer.DrawText(g, Compact(g, itemName, 261 - 10 - typeWidth - 52), Translator.IsKoreanStringPresent(itemName) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipDetailFont2, new Point(52, picHeight), ((SolidBrush)brush).Color, TextFormatFlags.NoPadding);
                ...
                if (isSilverWolf)
                {
                    TextRenderer.DrawText(g, "一件銀狼武器", GearGraphics.EquipDetailFont2, new Point(10, picHeight), ((SolidBrush)GearGraphics.GrayBrush2).Color, TextFormatFlags.NoPadding);
                    TextRenderer.DrawText(g, "(銀狼武器)", GearGraphics.EquipDetailFont2, new Point(252 - TextRenderer.MeasureText(g, "(銀狼)", GearGraphics.EquipDetailFont2, new Size(int.MaxValue, int.MaxValue), TextFormatFlags.NoPadding).Width, picHeight), ((SolidBrush)GearGraphics.GrayBrush2).Color, TextFormatFlags.NoPadding);
                    picHeight += 18;
                }
            }
            else
            ...
            foreach (KeyValuePair<int, SetItemEffect> effect in this.SetItem.Effects)
            {
                ...
                foreach (KeyValuePair<GearPropType, object> prop in props)
                {
                    if (prop.Key == GearPropType.Option)
                    {
                        ...
                        {
                            GearGraphics.DrawString(g, p.ConvertSummary(), Translator.IsKoreanStringPresent(p.ConvertSummary()) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipDetailFont2, new Dictionary<string, Color>() { { string.Empty, color } }, 10, 244, ref picHeight, 15);
                        }
                    }
