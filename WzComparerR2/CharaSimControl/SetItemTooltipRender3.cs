...
{
    public class SetItemTooltipRender3 : TooltipRender
    {
        public SetItemTooltipRender3()
        {
            ...
            foreach (KeyValuePair<int, SetItemEffect> effect in this.SetItem.Effects)
            {
                ...
                foreach (KeyValuePair<GearPropType, object> prop in props)
                {
                    if (prop.Key == GearPropType.Option)
                    {
                        ...
                        {
                            GearGraphics.DrawString(g, p.ConvertSummary(), Translator.IsKoreanStringPresent(p.ConvertSummary()) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipMDMoris9Font, new Dictionary<string, Color>() { { string.Empty, color } }, 14 + dx, 290, ref picHeight, 15);
                        }
                    }
                    ...
                    else if (prop.Key == GearPropType.activeSkill)
                    {
                        ...
                        foreach (SetItemActiveSkill p in ops)
                        {
                            ...
                            if (Translator.IsKoreanStringPresent(sr.Name))
                            {
                                ...
                                GearGraphics.DrawPlainText(g, summary, GearGraphics.KMSItemDetailFont2, color, 14 + dx, 290, ref picHeight, 15);
                            }
                            else
                            {
