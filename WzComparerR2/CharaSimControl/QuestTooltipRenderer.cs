...
namespace WzComparerR2.CharaSimControl
{
    public class QuestTooltipRenderer : TooltipRender
    {
        ...
        private Bitmap RenderBase()
        {
            ...
            if (!string.IsNullOrEmpty(this.Quest.Name))
            {
                if (Translator.IsTranslateEnabled)
                {
                    ...
                    TextRenderer.DrawText(g, translatedQuestName, Translator.IsKoreanStringPresent(translatedQuestName) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipMDMoris9Font, new Point(left, 72), Color.White, TextFormatFlags.NoPadding);
                }
                else
                {
                    TextRenderer.DrawText(g, Compact(g, this.Quest.Name, 200), Translator.IsKoreanStringPresent(this.Quest.Name) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipMDMoris9Font, new Point(left, 72), Color.White, TextFormatFlags.NoPadding);
                }
            }
            // 권장 레벨
            if (this.Quest.Lvmin > 0)
            {
                ...
                {
                    TextRenderer.DrawText(g, $"建議等級：{this.Quest.Lvmin}以上", GearGraphics.EquipMDMoris9Font, new Point(left, 90), ((SolidBrush)GearGraphics.QuestBrushDefault).Color, TextFormatFlags.NoPadding);
                }
                else
                {
                    var lvMax = this.Quest.Lvmax > 0 ? this.Quest.Lvmax : this.Quest.Lvmin + 4;
                    TextRenderer.DrawText(g, $"建議等級：{this.Quest.Lvmin}～{lvMax}", GearGraphics.EquipMDMoris9Font, new Point(left, 90), ((SolidBrush)GearGraphics.QuestBrushDefault).Color, TextFormatFlags.NoPadding);
                }
            }
            if (npcImage.Bitmap != null)
            {
            ...
            if (state == 1)
            {
                ...
                if (this.Quest.Check1NpcID > 0) target1.Add($@"去找{targetNpcPrefix}#$p{GetNpcName(this.Quest.Check1NpcID)}#");

                if (!string.IsNullOrEmpty(this.Quest.PlaceSummary)) target2.Add($@"執行位置：\n{this.Quest.PlaceSummary}");
                ...
                if (targetr.Count > 0)
                {
                    ...
                    GearGraphics.DrawString(g, replaced, Translator.IsKoreanStringPresent(replaced) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipMDMoris9Font, questColorTable, questFontTable, this.ImageTable, 29, 293, ref picH, 18, alignment: Text.TextAlignment.Left, defaultColor: ((SolidBrush)GearGraphics.QuestBrushDefault).Color);
                    ClearImageTable();
            ...
            if (!string.IsNullOrEmpty(desc))
            {
                ...
                switch (Translator.DefaultPreferredLayout)
                {
                    case 1:
                        ...
                        GearGraphics.DrawString(g, tReplaced, Translator.IsKoreanStringPresent(tReplaced) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipMDMoris9Font, questColorTable, questFontTable, this.ImageTable, 29, 293, ref picH, 18, alignment: Text.TextAlignment.Left, defaultColor: ((SolidBrush)GearGraphics.QuestBrushDefault).Color);
                        GearGraphics.DrawString(g, replaced, Translator.IsKoreanStringPresent(replaced) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipMDMoris9Font, questColorTable, questFontTable, this.ImageTable, 29, 293, ref picH, 18, alignment: Text.TextAlignment.Left, defaultColor: ((SolidBrush)GearGraphics.QuestBrushDefault).Color);
                        break;
                    case 2:
                        tReplaced = Translator.TranslateString(replaced) + "\r\n\r\n";
            ...
            // 상태
            var stateText = new string[] { "可開始", "進行中", "完成" };
            TextRenderer.DrawText(g2, $"狀態: {stateText[state]}" + (this.Quest.Blocked ? " / 不可執行任務" : ""), GearGraphics.EquipMDMoris9Font, new Point(21, picH + Margin_top - 26), ((SolidBrush)GearGraphics.QuestBrushDefault).Color, TextFormatFlags.NoPadding);
        ...
        {
            Size s = TextRenderer.MeasureText(g, text, Translator.IsKoreanStringPresent(text) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipMDMoris9Font, new Size(int.MaxValue, int.MaxValue), TextFormatFlags.NoPadding);
            ...
            while (seg > 1)
            {
                ...
                s = TextRenderer.MeasureText(g, tst, Translator.IsKoreanStringPresent(text) ? GearGraphics.KMSItemDetailFont2 : GearGraphics.EquipMDMoris9Font, new Size(int.MaxValue, int.MaxValue), TextFormatFlags.NoPadding);
