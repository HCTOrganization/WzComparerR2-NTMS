...
namespace WzComparerR2.Avatar.UI
{
    internal partial class AvatarForm : DevComponents.DotNetBar.OfficeForm
    {
            ...
            for (int i = 0; i < avatar.Parts.Length; i++)
            {
                ...
                if (part != null && part.Visible)
                {
                    ...
                    if (prismValid)
                    {
                        ...
                        partsID[i] += $"+{prismData.Type}h{prismData.Hue}s{prismData.Saturation}v{prismData.Brightness}{(prismData.ConvertPureBlack ? "PB" : "")}";
                    }
                    if (prism2Valid)
                    {
                        if (!prismValid)
        ...
        private void FillAvatarParts()
        {
            ...
            foreach (var part in avatar.Parts)
            {
                if (part != null)
                {
                    ...
                    {
                        var prismData = part.PrismData.Get(PrismDataCollection.PrismDataType.Default);
                        using Bitmap icon = Prism.Apply(part.Icon.Bitmap, prismData);
                        btn.SetIcon(icon, part.HasPrism);
                    }
                    btn.Tag = part;
                    ...
                    btn.chkConvertPureBlack.CheckedChanged += ChkConvertPureBlack_CheckedChanged;
            ...
            if (btn != null)
            {
                ...
                if (part != null)
                {
                    ...
                    if (part != avatar.Head && btn.PrismIndex == 0)
                    {
                        ...
                        using Bitmap icon = Prism.Apply(part.Icon.Bitmap, iconPrismData);
                        btn.SetIcon(icon, part.HasPrism);
                    }
                }
            }
        ...
        private void SliderSaturation_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (btn != null)
            {
                ...
                if (part != null)
                {
                    ...
                    labelSaturation.Text = $"飽和度({(slider.Value + 100 > 100 ? "+" : "")}{slider.Value + 100 - 100})";
                }
            }
        }
        ...
        private void SliderBrightness_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (btn != null)
            {
                ...
                if (part != null)
                {
                    ...
                    labelBrightness.Text = $"亮度({(slider.Value + 100 > 100 ? "+" : "")}{slider.Value + 100 - 100})";
                }
            }
        }
        ...
        private void ChkConvertPureBlack_CheckedChanged(object sender, EventArgs e)
        {
            var btn = (sender as BaseItem).Parent as AvatarPartButtonItem;
            if (btn != null)
            {
                ...
                if (part != null)
                {
                    btn.PrismConvertPureBlackChanged(btn.chkConvertPureBlack.Checked);
                    if (this.isUpdatingBtnItem) return;
                    this.UpdateDisplay();
                    this.SetButtonText(part, btn);
                    if (part != avatar.Head && btn.PrismIndex == 0)
            ...
            if (part.ID != null && (stringLinker.StringEqp.TryGetValue(part.ID.Value, out sr) || stringLinker.StringSkill.TryGetValue(part.ID.Value, out sr) || stringLinker.StringItem.TryGetValue(part.ID.Value, out sr)))
            {
                ...
                if (part.HasPrism)
                {
                    ...
                    if (part.PrismData.IsValid(PrismDataCollection.PrismDataType.WeaponEffect))
                    {
                        ...
                        (prismData2.ConvertPureBlack ? "PB" : ""));
                    }
                }
            }
            else
        ...
        private void btnBodyFloating_Click(object sender, EventArgs e)
        {
            ...
            {
                ToastNotification.Show(this, $"{nextAction} アクションが見つかりません。", null, 2000, eToastGlowColor.Red, eToastPosition.TopCenter);
            }
        }
        ...
        private void btnLaraTheSheep_Click(object sender, EventArgs e)
        {
            ...
            {
                case DialogResult.Yes:
                    LoadCode("2000,12000,51484,61183,1003968,1052667,1072867,1082552,1113329,1703113", 0);
                    return;
        ...
        private async void btnExportLwa_Click(object sender, EventArgs e)
        {
            ...
            {
                ToastNotification.Show(this, $"錯誤：使用此功能前，需要創建一個紙娃娃。", null, 2000, eToastGlowColor.Red, eToastPosition.TopCenter);
                return;
            }
        ...
        private async void btnAPI_Click(object sender, EventArgs e)
        {
            ...
            if (PluginManager.FindWz(Wz_Type.Base) == null)
            {
                ToastNotification.Show(this, $"錯誤：未開啟 Base.wz 檔案。", null, 2000, eToastGlowColor.Red, eToastPosition.TopCenter);
                return;
            }
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    default:
                        ToastNotification.Show(this, $"請選擇角色歸屬地區。", null, 3000, eToastGlowColor.Red, eToastPosition.TopCenter);
                        return;
                ...
                if (dlg.selectedRegion == 8)
                {
                    try
                    {
                        ToastNotification.Show(this, $"正在取得紙娃娃情報，請稍等...", null, 3000, eToastGlowColor.Green, eToastPosition.TopCenter);
...
#else
            ToastNotification.Show(this, $"若要使用此功能，您必須使用.NET 6.0 或 .NET 8.0 版本。", null, 2000, eToastGlowColor.Red, eToastPosition.TopCenter);
            return;
#endif
        }
        ...
        private string GetPrismCode(OpenAPI.PrismInfo prism, bool alwaysReturnCode = false)
        {
            if (prism.Valid)
            {
                ...
                {
                    return $"+{prism.ColorType}h{prism.Hue}s{prism.Saturation}v{prism.Brightness}{(prism.ConvertPureBlack ? "PB" : "")}";
                }
                else
                {
                    return $"+0h0s100v100";
        ...
        public void LoadCode(string code, int loadType)
        {
            ...
            if (matches.Count <= 0)
            {
                ToastNotification.Show(this, $"錯誤: 沒有與道具 ID 相符的道具。", null, 3000, eToastGlowColor.Red, eToastPosition.TopCenter);
                return;
            }
            ...
            {
                ToastNotification.Show(this, $"錯誤: 無法初始化紙娃娃模組。", null, 2000, eToastGlowColor.Red, eToastPosition.TopCenter);
                return;
            }
        ...
        {
            var prismMatches = Regex.Matches(prismStr, @"\+(\d+)h(\d+)s(\d+)v(\d+)(PB)?");
            if (prismMatches.Count >= 2)
            {
                ...
                if (Gear.IsWeapon(partType) || Gear.IsCashWeapon(partType))
                {
                    ...
                    {
                        part.PrismData.Set(PrismDataCollection.PrismDataType.WeaponEffect, type, hue, saturation, brightness, prismMatch.Groups[5].Success);
                    }
                }
            }
            if (prismMatches.Count >= 1)
            {
                ...
                if (Int32.TryParse(prismMatch.Groups[1].Value, out int type)
                    ...
                    && Int32.TryParse(prismMatch.Groups[4].Value, out int brightness))
                {
                    part.PrismData.Set(PrismDataCollection.PrismDataType.Default, type, hue, saturation, brightness, prismMatch.Groups[5].Success);
                }
            }
        }
